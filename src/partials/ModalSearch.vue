...
<template>
  ...
  >
    <div v-show="modalOpen" class="fixed inset-0 bg-gray-900/20 z-50 transition-opacity" aria-hidden="true"></div>
  </transition>
  <!-- Modal dialog -->
  <transition
    ...
    <div v-show="modalOpen" :id="id" class="fixed inset-0 z-50 overflow-hidden flex items-start top-20 md:top-28 mb-4 justify-center px-4 sm:px-6" role="dialog" aria-modal="true">
      <div ref="modalContent" class="bg-white overflow-auto max-w-2xl w-full max-h-full rounded-xl shadow-lg">
        ...
        <div class="p-2">
          <ul>
            <li>
              <router-link class="flex items-center px-2 py-1 leading-6 text-sm text-gray-700 hover:bg-gray-100 rounded-lg focus-within:bg-gray-100 outline-hidden" to="#0" @click="$emit('close-modal')">
